\hypertarget{Cache_8h}{\section{Riferimenti per il file Client/inc/\+Cache.h}
\label{Cache_8h}\index{Client/inc/\+Cache.\+h@{Client/inc/\+Cache.\+h}}
}
\subsection*{Funzioni}
\begin{DoxyCompactItemize}
\item 
F\+I\+L\+E $\ast$ \hyperlink{Cache_8h_ab85cfa2751c24894cd4bdca9254ed117}{create\+Temp\+File} (char $\ast$basename)
\item 
int \hyperlink{Cache_8h_a403785c0f2573dd16d81eff4ab26c20d}{read\+Request} (\hyperlink{structMyDFSId}{My\+D\+F\+S\+Id} $\ast$id, int pos, int size, \hyperlink{structCacheRequest}{Cache\+Request} $\ast$req)
\begin{DoxyCompactList}\small\item\em Usato per dire se il dato che voglio leggere si trova in cache oppure no. \end{DoxyCompactList}\item 
int \hyperlink{Cache_8h_a686e517ab500f68c40c03bfbc3e2e2fd}{write\+Cache} (\hyperlink{structMyDFSId}{My\+D\+F\+S\+Id} $\ast$id, void $\ast$buffer, int size, int pos)
\begin{DoxyCompactList}\small\item\em Funzione che scrive nel file di cache il contenuto di una read, senza sovrascrivere le parti scritte da write locali. \end{DoxyCompactList}\end{DoxyCompactItemize}


\subsection{Documentazione delle funzioni}
\hypertarget{Cache_8h_ab85cfa2751c24894cd4bdca9254ed117}{\index{Cache.\+h@{Cache.\+h}!create\+Temp\+File@{create\+Temp\+File}}
\index{create\+Temp\+File@{create\+Temp\+File}!Cache.\+h@{Cache.\+h}}
\subsubsection[{create\+Temp\+File}]{\setlength{\rightskip}{0pt plus 5cm}F\+I\+L\+E$\ast$ create\+Temp\+File (
\begin{DoxyParamCaption}
\item[{char $\ast$}]{basename}
\end{DoxyParamCaption}
)}}\label{Cache_8h_ab85cfa2751c24894cd4bdca9254ed117}


Referenziato da main(), e mydfs\+\_\+open().

\hypertarget{Cache_8h_a403785c0f2573dd16d81eff4ab26c20d}{\index{Cache.\+h@{Cache.\+h}!read\+Request@{read\+Request}}
\index{read\+Request@{read\+Request}!Cache.\+h@{Cache.\+h}}
\subsubsection[{read\+Request}]{\setlength{\rightskip}{0pt plus 5cm}int read\+Request (
\begin{DoxyParamCaption}
\item[{{\bf My\+D\+F\+S\+Id} $\ast$}]{id, }
\item[{int}]{pos, }
\item[{int}]{size, }
\item[{{\bf Cache\+Request} $\ast$}]{req}
\end{DoxyParamCaption}
)}}\label{Cache_8h_a403785c0f2573dd16d81eff4ab26c20d}


Usato per dire se il dato che voglio leggere si trova in cache oppure no. 

\begin{DoxyReturn}{Restituisce}
n M\+I\+S\+S\+: In caso di M\+I\+S\+S, dentro a req verranno messi posizione e size del dato da scaricare. 

0 H\+I\+T\+: per interrompere il while, torna false.
\end{DoxyReturn}
Nota\+: La miss si riferisce ad un \char`\"{}buco\char`\"{}, quindi bisogna chiamare la funzione piu' volte per assicurarsi che non ci siano altri miss. \begin{DoxyRefDesc}{Da fare}
\item[\hyperlink{todo__todo000001}{Da fare}]da terminare \end{DoxyRefDesc}


Referenzia Write\+Op\+::next, Read\+Op\+::next, Write\+Op\+::pos, Read\+Op\+::pos, Cache\+Request\+::pos, read\+Request(), Write\+Op\+::size, Read\+Op\+::size, e Cache\+Request\+::size.



Referenziato da mydfs\+\_\+read(), e read\+Request().

\hypertarget{Cache_8h_a686e517ab500f68c40c03bfbc3e2e2fd}{\index{Cache.\+h@{Cache.\+h}!write\+Cache@{write\+Cache}}
\index{write\+Cache@{write\+Cache}!Cache.\+h@{Cache.\+h}}
\subsubsection[{write\+Cache}]{\setlength{\rightskip}{0pt plus 5cm}int write\+Cache (
\begin{DoxyParamCaption}
\item[{{\bf My\+D\+F\+S\+Id} $\ast$}]{id, }
\item[{void $\ast$}]{buffer, }
\item[{int}]{size, }
\item[{int}]{pos}
\end{DoxyParamCaption}
)}}\label{Cache_8h_a686e517ab500f68c40c03bfbc3e2e2fd}


Funzione che scrive nel file di cache il contenuto di una read, senza sovrascrivere le parti scritte da write locali. 

Eseguo un ciclo che vede se il punto in cui\+:
\begin{DoxyEnumerate}
\item Controllo se il punto in cui voglio scrivere e' contenuto in una operazione di write. In quel caso, richiamo questa funzione spostando avanti il puntatore del buffer
\item Il punto in cui voglio scrivere non e' incluso in nessuna write. In questo caso, vedo se c'e' una write inclusa nello spazio del buffer della read che voglio scrivere. Se e' cosi', scrivo quanto posso, e se posso scrivere altro dopo la sezione di write, richiamo questa funzione ricorsivamente aggiornado gli indici. \begin{DoxyReturn}{Restituisce}
1 se tutto e' ok, 0 se qualcosa e' andata storta 
\end{DoxyReturn}

\end{DoxyEnumerate}

Referenzia My\+D\+F\+S\+Id\+::fp, log\+M(), Write\+Op\+::next, Write\+Op\+::pos, Write\+Op\+::size, e write\+Cache().



Referenziato da read\+From(), e write\+Cache().

